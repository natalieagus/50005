...
2. SecureStore server (you) **uploads** the certificate signing request (`.csr` file) for approval to csertificate, our CA. **Our CA is integrated with our CS Telegram bot** (@cssubmitbot). <span style="color:#f77729;"><b>Simply type `/start` to our bot and follow the instructions.</b></span> Our CA will:
...
We will <span style="color:#f77729;"><b>manually</b></span> check the implementation of your `MODE 3` in both Client and Server scripts. This will be done during the demo (consult the checkoff schedule link on eDimension) 
